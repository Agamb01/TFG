...
%
%Algunas técnicas de tolerancia frente a estos fallos se analizan en el apartado \ref{cap2:tolerancia:estatica}.
%
%-------------------------------------------------------------------
...
%Existen otros tipos de fallos permanentes, \textit{Single-Event Burnout (SEB)} y \textit{Single-Event Gate Rupture (SEGR)}, que destruyen el transistor a nivel físico.	
  %
%Los fallos permanentes, una vez detectados, unicamente pueden solucionarse sustituyendo el chip o moviendolo de un lugar a otro dentro del mismo chip, véase el apartado de \ref{cap2:tolerancia:dinamica}.
...
Aquellos fallos que no se enmascaran por no usarse el segmento donde se producen pueden causar graves consecuencias, para ello existen técnicas de tolerancia a fallos que se comentan a continuación.

Cuando ocurren fallos de los comentados en la sección \ref{cap2:fallos} pueden no llegan a provocar errores en el funcionamiento del sistema. Los cambios provocados por los fallos se ven enmascarados por las siguientes razones:

\begin{itemize}

  \item \textbf{Enmascarado lógico}: Se evita el error en una puerta lógica, gracias a que el valor del dato no es necesario para estimar la salida. En la figura \ref{fig:Logical_masking} vemos que el valor de la señal invertida es indiferente para calcular el resultado ya que el resultado de una puerta "`or"' es "`1"' siempre que una de sus entradas sea "`1"'.
	
	\item \textbf{Enmascarado eléctrico}: El fallo producido pierde intensidad en el recorrido lógico y no tiene efecto al llegar al elemento de memoria que lo almacenaría. Figura \ref{fig:Electrical_masking}.
	
	\item \textbf{Enmascarado temporal}: El fallo se propaga con suficiente energía hasta el biestable, pero no se almacena debido a que no ocurre en la ventana de reloj.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.75\textwidth]{Imagenes/SEU/Logical_masking.gif}
...
\begin{figure}[htbp]
	...
	\caption{Fallo enmascarado por electricamente.}
	\label{fig:Electrical_masking}
\end{figure}

\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.75\textwidth]{Imagenes/SEU/Latching_masking.gif}
	\caption{Fallo enmascarado por tiempo.}
	\label{fig:Latching_masking}
\end{figure}



Para poder tolerar los fallos vistos en la sección \ref{cap2:fallos} existen dos tipos de tolerancia; tolerancia estática y tolerancia dinámica.

%-------------------------------------------------------------------
\subsection{Tolerancia estática}
%-------------------------------------------------------------------
\label{cap2:tolerancia:estatica}

...

%-------------------------------------------------------------------
\subsection{Tolerancia dinámica}
%-------------------------------------------------------------------
\label{cap2:tolerancia:dinamica}
